...
1. `cell_features_500m.pkl`等、「集計特徴量ファイル」だと思われていたものが
...
当初は旧版での結論(視界特徴に精度面の追加価値なし、閾値0.086未達)を
根拠に優先度を下げていたが、これは誤りだった。理由: ((1) std/pale比較で
...
│   └── tokyo_pedestrian_accidents.csv  # 旧リポジトリから複製
...
- [x] 一致セット(旧版の凍結manifestからcell_id・accident_countのみ借用、
      画像・OSM特徴量は全て自前パイプラインで再計算)でGSI版stdを再評価 →
      統計的に確定的に劣ると判明(bootstrap CI [-0.156,-0.048])
...
- [x] 23区視界特徴評価をGSI版パイプライン単体でやり直し(旧版の結論を
...
HuggingFace Spaces)は実は**旧版モデルのまま**で、`get_point_facts`の一部・
