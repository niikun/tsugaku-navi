...
インフラセルによる水増しと判明(実際は0.34〜0.35程度)。CNN+視界stacking
の劣化(-0.079)はGBDT量子化アーティファクトと判明(線形コンバイナでは
-0.001)。条件付き分散異常(1049.65)は既知の数学的アーティファクト(2セル
限定、順位指標への影響なし)と確定診断。3件とも「ユーザーが見出し数値を
鵜呑みにせず追加検証を要求した」ことで発覚・解決した。詳細な経緯・全数値は
...
- [x] **全域0.439は山間部・低インフラセルで水増しされていたと訂正**:
      ...
      ほぼ収束(交差検証的な確認)。**密集市街地での実際の判別力は0.34〜
      0.35程度**が正直な数字。企画書・Q&Aでは0.439ではなくこちらを使う。
      std/pale選定判断・本番モデルの学習自体には影響なし
...
- [x] **CNN予測値の条件付き分散異常(1049.65)を確定診断**: std版と完全に
      ...
      確認。pale・7年データ・全量という今回の変更由来の新しい問題ではない
      (eval側のセル構成・OSM特徴量はstd/pale間で不変なため)
- [x] **技術検証はこれで打ち止め**(ユーザー確認済み)。詳細な検証過程・
      数値は全て`STUDY_LOG.md`に記録済み
