...
def closing_slide(prs, page_no):
    ...
    set_run(
        p2,
        "「東京都オープンデータ×AIをもとに、データ作成、追加学習を行い、"
...
def build():
    ...
    content_slide(prs, page, "2", "課題背景(社会課題の提示)", [
        ...
        (0, "新宿区だけでも通学路交通安全プログラムの総点検で毎年数十箇所の対策が必要と判明しており(令和5年度41箇所)、行政の定期点検だけでは網羅しきれない", None),
    ...
    content_slide(prs, page, "4", "活用するオープンデータ", [
        (0, "都内15区教育機関一覧(東京都オープンデータカタログサイト): 区立小中学校773校の位置を地図上に常時表示", None),
    ...
    content_slide(prs, page, "10", "今後の展望・拡張性", [
        (0, "東京都オープンデータのさらなる活用(教育機関一覧の未対応区への拡大、道路交通センサス等を組み合わせた特徴量追加)", None),
