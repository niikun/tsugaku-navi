...
設計方針(「分離案」を踏襲): CNNのスコアはルーティング
(どこが危険かのランキング)にのみ使い、「なぜ危険か」の説明はモデルの内部を
可視化する(Grad-CAM等)のではなく、その地点の事実情報(OSM属性・視界特徴の
実値)を使って組み立てる。

CATEGORY_THRESHOLDSは、GSI版train全域(3,803セル、非拡張)へのアンサンブル予測から
改めてパーセンタイルを計算すること(`recompute_category_thresholds()`参照)。
学習データ年数範囲・タイル種別が異なれば予測値の分布も変わるため、値の使い回しはしない。
"""
import math
import os
